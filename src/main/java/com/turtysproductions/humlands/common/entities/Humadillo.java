...
{
	private static final DataParameter<Boolean> CLOSED = EntityDataManager.createKey(Humadillo.class, DataSerializers.BOOLEAN);
	   private static final DataParameter<Boolean> RIDDEN = EntityDataManager.createKey(Humadillo.class, DataSerializers.BOOLEAN);
	...
	//When the timer's over, the humadillo is out of his shell, the timer lasts 20 seconds, 
	//in which he will have resistance, movement speed set to zero and he will be in his shell.
	@Override
	...
	//Checks if the player right clicks the Humadillo with a mushroom, and if he does, the entity is tamed and rideable.
	@Override
	public boolean processInteract(PlayerEntity player, Hand hand) 
	{
		...
		if(item == Items.BROWN_MUSHROOM)
        {
	    	   ...
	    	   world.addParticle(ParticleTypes.HEART, this.getPosXRandom(1.0D), this.getPosYRandom() + 0.5D, this.getPosZRandom(1.0D), d0, d1, d2);
	    	   return true;
        }
