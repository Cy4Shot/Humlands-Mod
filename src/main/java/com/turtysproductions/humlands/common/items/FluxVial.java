public class FluxVial extends Item
{
	            ...
	            if (this.getFluid() == Fluids.EMPTY) {
	               ...
	               if (blockstate1.getBlock() instanceof IBucketPickupHandler) {
	                  ...
	                  if (fluid != Fluids.EMPTY) {
	                     ...
	                     if (soundevent == null) soundevent = fluid.isIn(FluidInit.Tags.FLUX) ? SoundEvents.ITEM_BUCKET_FILL_LAVA : SoundEvents.ITEM_BUCKET_FILL;
	                     playerIn.playSound(soundevent, 1.0F, 1.0F);
	                     ItemStack itemstack1 = this.fillBucket(itemstack, playerIn, fluid.getFilledBucket());
	                     if (!worldIn.isRemote) {
	                        ModTriggers.FILLED_VIAL.trigger((ServerPlayerEntity)playerIn, new ItemStack(fluid.getFilledBucket()));
	                     }
	               ...
	               BlockPos blockpos2 = blockstate.getBlock() instanceof ILiquidContainer && this.getFluid() == FluidInit.flux_fluid.get() ? blockpos : blockpos1;
	...
	public boolean tryPlaceContainedLiquid(@Nullable PlayerEntity player, World worldIn, BlockPos posIn, @Nullable BlockRayTraceResult blockraytraceresult) {
		      if (!(this.getFluid() instanceof FlowingFluid)) {
		         return false;
		          ...
		          if (blockstate.isAir() || flag || blockstate.getBlock() instanceof ILiquidContainer && ((ILiquidContainer)blockstate.getBlock()).canContainFluid(worldIn, posIn, blockstate, this.getFluid())) {
		             if (worldIn.dimension.doesWaterVaporize() && this.getFluid().isIn(FluidInit.Tags.FLUX)) {
		                ...
		                for(int l = 0; l < 8; ++l) {
		                 ...
		                 else if (blockstate.getBlock() instanceof ILiquidContainer && this.getFluid() == FluidInit.flux_fluid.get()) 
		            {
		               if (((ILiquidContainer)blockstate.getBlock()).receiveFluid(worldIn, posIn, blockstate, ((FlowingFluid)this.getFluid()).getStillFluidState(false))) 
		               {
		            ...
		            return blockraytraceresult == null ? false : this.tryPlaceContainedLiquid(player, worldIn, blockraytraceresult.getPos().offset(blockraytraceresult.getFace()), (BlockRayTraceResult)null);
		         }
		      }
}
		   ...
		   protected void playEmptySound(@Nullable PlayerEntity player, IWorld worldIn, BlockPos pos) {
		      SoundEvent soundevent = this.getFluid().getAttributes().getEmptySound();
		      if(soundevent == null) soundevent = this.getFluid().isIn(FluidInit.Tags.FLUX) ? SoundEvents.ITEM_BUCKET_EMPTY_LAVA : SoundEvents.ITEM_BUCKET_EMPTY;
