...
public class HumlandsFossilFeature extends Feature<NoFeatureConfig> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, NoFeatureConfig config) {
      ...
      int y1 = worldIn.getHeight();
      
      boolean foundGround = false;
		
      while(!foundGround && y1-- >= 0)
      {
			Block block = worldIn.getBlockState(new BlockPos(j, y1, k)).getBlock();
			foundGround = block.isAir(worldIn.getBlockState(new BlockPos(j, y1, k)), worldIn, new BlockPos(j, y1, k));
      }
      ...
      int k1 = j;
      BlockPos blockpos1 = template.getZeroPositionWithTransform(pos.add(j, y1, k), Mirror.NONE, rotation);
